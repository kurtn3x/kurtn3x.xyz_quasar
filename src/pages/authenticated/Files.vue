...
import { getIcon, createTypes } from 'src/components/lib/mimeMap';
...
import { enableDragDropTouch } from 'src/components/lib/drag-drop-touch.esm.min.js';
  ...
  methods: {
    ...
    getHomeFolder() {
      ...
      apiGet('/files/folder/home', this.axiosConfig).then((apiData) => {
        if (apiData.error == false) {
          ...
          this.q.notify({ type: 'negative', message: apiData.errorMessage });
    ...
    createFolder(name: string) {
      ...
      apiPost('/files/folder', data, this.axiosConfig).then((apiData) => {
        if (apiData.error == false) {
          ...
          this.q.notify({ type: 'positive', message: 'Created' });
          this.refreshFolder();
          this.resetFilterState();
        } else {
          this.q.notify({ type: 'negative', message: apiData.errorMessage });
    ...
    moveSelection(newParentId: string) {
      if (newParentId != this.rawFolderContent.id) {
        ...
        this.q.notify({ type: 'positive', message: 'Nothing to move' });
      }
    ...
    updateParent(itemType: string, itemId: string, newParentId: string) {
      ...
      if (newParentId != itemId) {
        ...
        apiPut(
          ...
          if (apiData.error == false) {
            this.q.notify({ type: 'positive', message: 'Updated' });
            this.refreshFolder();
            this.resetFilterState();
            this.selectedItems = [];
            this.allSelected = false;
